...
(function () {
  ...
  function closeVideoHub(toVideoSec) {
    const vh = document.getElementById('video-hub');
    if (vh) {
      vh.classList.add('hidden');
      setTimeout(() => { vh.style.display = 'none'; vh.classList.remove('hidden'); }, 500);
    }
    // ESC로 나갈 때는 영상(VIDEO) 항목 타이틀로 복귀
    if (toVideoSec && window.spReturnTo) window.spReturnTo(3);
    const mainHub = document.getElementById('hub');
    if (mainHub) {
  ...
  function buildEditor() {
    ...
    el.innerHTML =
      '<div class="vs-top">'
      + '<div class="vs-brand">'
      + '<button class="vs-back" id="vs-back"><svg width="7" height="12" viewBox="0 0 7 12" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><polyline points="6 1 1 6 6 11"/></svg> 뒤로</button>'
