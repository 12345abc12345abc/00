...
function renderCover(s, idx, total) {
  ...
  return mkS(
    ...
    // faint top + bottom edge hairlines for a framed, gallery feel
    + '<div style="position:absolute;inset:0;display:flex;flex-direction:column;padding:86px 104px 74px">'

      // ── eyebrow: red diamond · tag · trailing rule ──
      + '<div class="a-up d0" style="display:flex;align-items:center;gap:18px">'
        + '<span style="flex:none;width:9px;height:9px;background:var(--ieg-red);transform:rotate(45deg)"></span>'
        + '<span style="font-size:13px;font-weight:700;letter-spacing:0.3em;color:var(--ieg-red);text-transform:uppercase;white-space:nowrap">' + esc(d.tag) + '</span>'
        + '<span class="a-line d2" style="flex:1;height:1px;background:linear-gradient(90deg,rgba(255,255,255,0.22),rgba(255,255,255,0));transform-origin:left center"></span>'
      + '</div>'

      // ── hero block (vertically centered) ──
      + '<div style="flex:1;display:flex;flex-direction:column;justify-content:center;min-height:0">'
        + '<h1 class="a-rise d2" style="font-size:98px;font-weight:600;line-height:1.0;letter-spacing:-0.04em;color:#fff;margin:0;max-width:1000px;text-wrap:balance">' + esc(d.heading) + '</h1>'
        + '<div class="a-up d4" style="margin-top:26px;font-size:27px;font-weight:600;letter-spacing:-0.012em;color:rgba(255,255,255,0.55)">' + esc(d.company) + '</div>'
        + '<span class="a-line d5" style="display:block;width:92px;height:4px;background:var(--ieg-red);border-radius:2px;margin-top:40px;transform-origin:left center"></span>'
      + '</div>'

      // ── footer meta: hairline + address / contact ──
      + '<div class="a-up d6">'
        + '<div style="height:1px;background:rgba(255,255,255,0.13);margin-bottom:22px"></div>'
        + '<div style="display:flex;align-items:flex-end;justify-content:space-between;gap:40px">'
          + '<div style="font-size:12.5px;font-weight:500;line-height:1.7;letter-spacing:0.004em;color:rgba(255,255,255,0.5);max-width:660px">' + esc(d.addr) + '</div>'
          + '<div style="font-size:12.5px;font-weight:500;line-height:1.7;letter-spacing:0.01em;color:rgba(255,255,255,0.5);text-align:right;white-space:nowrap">' + esc(d.contact) + '</div>'
        + '</div>'
      + '</div>'
...
function renderCornerstone(s, idx, total) {
  ...
  // expansion companies → clean hairline-card gallery grid (design-system aligned)
  const branches = list.map((b, i) => {
    const num = ('0' + (i + 1)).slice(-2);
    return '<div class="a-up d' + Math.min(i + 4, 9) + '" style="background:var(--canvas);border:1px solid var(--hairline);border-radius:14px;padding:15px 17px;display:flex;align-items:flex-start;gap:13px;min-height:0">'
      + '<div style="font-size:12px;font-weight:700;color:var(--ieg-red);font-variant-numeric:tabular-nums;letter-spacing:0.02em;line-height:1.5;flex-shrink:0">' + num + '</div>'
      + '<div style="min-width:0">'
        + '<div style="font-size:13.5px;font-weight:700;color:var(--ink);letter-spacing:-0.014em;line-height:1.3">' + esc(b.label) + '</div>'
        + '<div style="font-size:11px;color:var(--ink-muted-48);line-height:1.45;margin-top:3px;letter-spacing:-0.004em">' + esc(b.desc) + '</div>'
      + '</div>'
    + '</div>';
  }).join('');
  ...
  return mkS(
    ...
    + '<div class="slide-pad">'
    + HEAD(d.badge, d.headline)
    + '<div style="flex:1;display:flex;gap:34px;min-height:0;align-items:stretch">'
      // left: flat dark anchor tile (the cornerstone)
      + '<div class="a-r d3" style="width:332px;flex-shrink:0;display:flex;flex-direction:column;justify-content:center;padding:34px 30px;background:var(--tile-dark);border-radius:18px;position:relative">'
        + '<div class="kicker" style="color:var(--ieg-red)">CORNERSTONE</div>'
        + '<div style="font-size:30px;font-weight:700;color:#fff;letter-spacing:-0.028em;margin-top:10px;line-height:1.16;word-break:keep-all">' + esc(d.anchor.title) + '</div>'
        + '<div style="font-size:12px;color:rgba(255,255,255,0.5);margin-top:8px;letter-spacing:0.12em;font-weight:600">' + esc(d.anchor.sub) + '</div>'
        + '<div style="height:1px;background:rgba(255,255,255,0.12);margin:20px 0"></div>'
        + '<div style="font-size:12.5px;line-height:1.72;color:rgba(255,255,255,0.82);letter-spacing:-0.004em">' + esc(d.anchor.desc) + '</div>'
        + '<div style="margin-top:26px;display:flex;align-items:center;gap:11px">'
          + '<span style="width:34px;height:2px;background:var(--ieg-red);border-radius:2px"></span>'
          + '<span style="font-size:12px;font-weight:700;color:#fff;letter-spacing:-0.01em">' + list.length + '개 그룹사로 확장</span>'
        + '</div>'
      + '</div>'
      // right: expansion gallery — quiet header + 2×4 card grid
      + '<div style="flex:1;display:flex;flex-direction:column;min-width:0;min-height:0">'
        + '<div class="a-up d3" style="display:flex;align-items:center;gap:12px;margin-bottom:16px">'
          + '<span style="font-size:11px;font-weight:700;letter-spacing:0.26em;color:var(--ieg-red)">EXPANSION</span>'
          + '<span style="font-size:12px;font-weight:600;color:var(--ink-muted-48);letter-spacing:-0.006em">확장 그룹사</span>'
          + '<span style="flex:1;height:1px;background:var(--hairline)"></span>'
        + '</div>'
        + '<div style="flex:1;display:grid;grid-template-columns:1fr 1fr;grid-auto-rows:1fr;gap:12px;min-height:0">' + branches + '</div>'
      + '</div>'
    + '</div>'
    + '</div>'
    + FOOT(idx, total)
